# Site infrastructure + private Telegram chat system
...
A production system designed, built, and operated end-to-end: a static Hugo site paired with a small serverless backend (Cloudflare Worker + D1) that turns website visits into a private Telegram conversation, plus a legacy contact form the same backend supports. It runs with the same operational discipline as the infrastructure and automation work described on the site itself — every change is gated by CI (tests, typecheck, build) before it merges, the architecture has a documented security review and audit trail (`SECURITY-AUDIT.md`, `ROADMAP.md`), production errors are monitored through Sentry, and a data-deletion code path stays in tested dry-run/log-only mode until its output has been verified against real data before being enabled. Website visitors never interact with Telegram directly — the Worker stores conversations and callback requests in D1, sends owner notifications through the Telegram Bot API, and accepts owner replies through a secret-protected Telegram webhook. This document describes how the whole thing fits together; the domain it happens to be deployed on right now isn't load-bearing to any of it, and the same pattern works unchanged under a different one.
